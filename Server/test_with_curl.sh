...
if [ "$SELECTED_TASK" -eq 1 ]; then
  ...
  ENCRYPTED_INPUT="@client_files/ad_targeting/727.user_profile.input.fheencrypted"
  ENCRYPTED_OUTPUT="ad_targeting.output.fheencrypted"
  timeout=$((60 * 2))
  COMPLETED_TASK_ID="ca25eaaa-c8f1-4853-b214-30db2bad0695"
  ...
  timeout=$((60 * 5))
  COMPLETED_TASK_ID="9bb35455-9cbd-4b7d-a803-67e69ff64c65"
  ...
  timeout=$((60 * 5))
  COMPLETED_TASK_ID="ed5807b3-08f2-42d1-ad5d-dbd94c077c3b"
...
if [ "$GET_STATUS_ACTION" = "true" ]; then
    ...
    if [ "$COMPLETED_TASKS" = "true" ]; then

        ############# Test 1
        response_v1=$(curl -s -X GET "$URL/get_task_status?task_id=$COMPLETED_TASK_ID&uid=$COMPLETED_UID")
...
if [ "$GET_TASK_RESULT_ACTION" = "true" ]; then
    ...
    if [ "$COMPLETED_TASKS" = "true" ]; then
        ############## Test 1
        echo "📩 Completed 'TASK_ID': '$COMPLETED_TASK_ID'"
        check_task_result "$COMPLETED_TASK_ID" "$COMPLETED_UID"
        if [[ $? -ne 0 ]]; then
            echo "❌ Test failed!";
        else
            echo "🎉 Test passed";
